...
class BrainCore:
    ...
    def __init__(self):
        ...
        # Initialize services based on availability, but disable Gemini to prevent quota errors
        self.hf_service = HuggingFaceService() if settings.HUGGINGFACE_AVAILABLE else None
        self.gemini_service = None  # Disable Gemini API to prevent quota errors
    ...
    async def _analyze_with_ai(self, processed_text: str, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "possible_causes": ["AI services not configured"],
                "recommended_steps": [
                    {"step_number": 1, "description": "Configure AI service credentials"},
                    {"step_number": 2, "description": "Try again when services are available"}
                ],
                "confidence_score": 0.3,
                "additional_info": "AI services are not available. Using basic analysis only.",
                "external_sources": [],
